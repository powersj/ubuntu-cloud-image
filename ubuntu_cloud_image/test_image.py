...
def test_image_minimal():
    ...
    assert image.mirror_url == ("https://cloud-images.ubuntu.com/minimal/releases/")
    assert not image.daily
    assert image.minimal
...
def test_aws_daily():
    ...
    assert str(image) == ("daily %s (%s) image for AWS (%s)" % (release, arch, region))
    assert image.filter == [
        "arch=%s" % arch,
        "endpoint=https://ec2.%s.amazonaws.com" % region,
        "region=%s" % region,
...
def test_aws_china():
    ...
    assert str(image) == ("%s (%s) image for AWS China (%s)" % (release, arch, region))
    assert image.filter == [
        "arch=%s" % arch,
...
def test_azure():
    ...
    assert str(image) == ("%s (%s) image for Azure (%s)" % (release, arch, region))
    assert image.filter == [
        "arch=%s" % arch,
...
def test_gce_daily():
    ...
    assert str(image) == ("daily %s (%s) image for GCE (%s)" % (release, arch, region))
    assert image.filter == [
        "arch=%s" % arch,
        "endpoint=https://www.googleapis.com",
        "region=%s" % region,
